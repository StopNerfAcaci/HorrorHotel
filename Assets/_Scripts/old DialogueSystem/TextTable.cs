	...
	[CreateAssetMenu(fileName = "TextTable", menuName = "HorrorHotel/TextTable")]
	public class TextTable : ScriptableObject, ISerializationCallbackReceiver
	{
		[Serializable]
		private struct LanguageKeyValuePair
		{
			public string key;
		...
		private Dictionary<string, int> m_languages = new();

		private Dictionary<int, TextTableField> m_fields = new();

		[SerializeField]
		private List<string> m_languageKeys = new();

		[SerializeField]
		private List<int> m_languageValues = new ();

		[SerializeField]
		private List<int> m_fieldKeys = new();

		[SerializeField]
		private List<TextTableField> m_fieldValues = new();
		...
		public void SetFieldTextForLanguage(int fieldID, int languageID, string text)
		{
			if (!HasLanguage(languageID))
			{
				...
				{
					Debug.LogWarning((object)("TextTable.SetLanguageText(" + fieldID + ", " + languageID + ", \"" + text + "\") failed: Language doesn't exist. Use Text Table Editor or AddLanguage() to add the language first."), (Object)(object)this);
				}
				return;
			}
			TextTableField field = GetField(fieldID);
			if (field == null)
			{
				if (Debug.isDebugBuild)
				{
					Debug.LogWarning((object)("TextTable.SetLanguageText(" + fieldID + ", " + languageID + ", \"" + text + "\") failed: Field doesn't exist. Use Text Table Editor or AddField() to add the field first."), (Object)(object)this);
				}
			}
			else
			{
